...
SELECT * FROM EMPLOYEES WHERE manager_id IN(SELECT employee_id FROM EMPLOYEES WHERE manager_id IN (select employee_id from employees WHERE manager_id =1)) AND
employee_id != 1 ORDER BY 3;
with recursive managers as (SELECT employee_id, manager_id FROM employees WHERE employee_id = 1
							UNION ALL
                            SELECT e.employee_id, m.manager_id FROM managers m INNER JOIN  employees e ON e.manager_id = m.employee_id )
SELECT * FROM managers WHERE employee_id <> manager_id;
...
SELECT 1, 1 
union all
SELECT sr_no+1, n+2 from odd_no where sr_no < 10
)
SELECT * FROM odd_no; 
...
with cte as (select *, substr(trans_date, 1,7) as yyyy_mm  from transactions)
select yyyy_mm, country, count(*) as trans_count, count(if(state ='APPROVED', 1,NULL)) as approved_count, sum(amount) as trans_total_amount, 
	sum(if(state ='APPROVED', amount,0)) approved_total_amount from cte group by yyyy_mm,country ;
...
INSERT INTO actions VALUES(1,1,'2019-07-01','VIEW','NULL'),
        (1,1,'2019-07-01','LIKE','NULL'),(1,1,'2019-07-01','SHARE','NULL'),(2,2,'2019-07-04','VIEW','NULL'),(2,2,'2019-07-04','REPORT','SPAM'),
        (3,4,'2019-07-04','VIEW','NULL'),(3,4,'2019-07-04','REPORT','SPAM'),(4,3,'2019-07-02','VIEW','NULL'),(4,3,'2019-07-02','REPORT','SPAM'),
        (5,2,'2019-07-03','VIEW','NULL'),(5,2,'2019-07-03','REPORT','RACISM'),(5,5,'2019-07-03','VIEW','NULL'),(5,5,'2019-07-03','REPORT','RACISM');
INSERT INTO removals VALUES(2,'2019-07-20'),(3,'2019-07-18');

with spm as(select * from actions where extra = 'SPAM'),
spm_reported as (select action_date, post_id, count(extra) as daily_spm from spm group by action_date),
spm_removed as ( select post_id, count(remove_date) as no_id_rm from removals group by post_id ),
dp as (select (no_id_rm/daily_spm)*100 as daily_percent from spm_reported sr join spm_removed sm on sr.post_id = sm.post_id)
select round((sum(daily_percent)/count(daily_percent)), 2) as average_daily_percent from dp;

WITH temp_action AS (SELECT action_date, post_id, COUNT(EXTRA) OVER(PARTITION BY action_date) num_post_reported_spam FROM actions WHERE extra = 'SPAM')
SELECT ROUND(AVG(percentage), 2) AS avg_daily_percent FROM 
(SELECT action_date, ROUND((COUNT(post_id)/num_post_reported_spam) * 100, 2) AS percentage 
FROM temp_action WHERE post_id IN (SELECT post_id FROM removals)GROUP BY action_date ) temp;
